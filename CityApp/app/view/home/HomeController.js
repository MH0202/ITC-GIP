...
Ext.define('CityApp.view.home.HomeController', {
  ...
  onFocusChange: function (appState) {
    ...
    if (activeTab.id == 'transport_view') {
      appState.type == 'focus'
        ? txpCtrl.refreshVehicleLayer()
        : clearInterval(txpCtrl.interval);
    }
  },
  /*---*/
  ...
  onTabPanelChange: function (tabpanel, newTab, oldTab) {
    ...
    if (newTab.id == 'transport_view') {
      txpCtrl.deployed
        ? txpCtrl.refreshVehicleLayer()
        : (txpCtrl.deployed = true);
    } else {
      if (oldTab.id == 'transport_view') {
        clearInterval(txpCtrl.interval);
      }
    }
  },
  ...
  createMap: function (baseMap, mapPanel) {
    ...
    var tileUrlFunction = function (zxy) {
      /* zxy = tileCoord */
      if (zxy[1] < 0 || zxy[2] < 0) {
        return '';
      }
    ...
    var nlExtent = [-285401.92, 22598.08, 595401.92, 903401.92],
    ...
    var newMap = new ol.Map({
      ...
      view: new ol.View({
        ...
        center: [
          150000,
          450000,
        ] /* Default coordinates for the whole country */,
        minZoom: 2,
        maxZoom: 13,
        zoom: 3,
      }),
    });
    newMap.addControl(new ol.control.Zoom());
