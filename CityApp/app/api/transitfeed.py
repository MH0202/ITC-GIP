...
query.execute("""
    SELECT ST_XMin (ST_Extent((ST_Envelope(geom)))) as XMin, 
ST_XMax (ST_Extent((ST_Envelope(geom))))  AS XMax,
ST_YMin (ST_Extent((ST_Envelope(geom)))) as YMin, 
ST_YMax (ST_Extent((ST_Envelope(geom))))  AS YMax
FROM netherlands.municipality where gm_naam = 'Delft'
""")
Box = []
box = query.fetchall()
Box.append(box[0]['xmin'])
Box.append(box[0]['ymin'])
Box.append(box[0]['xmax'])
Box.append(box[0]['ymax'])
...
response = urllib.request.urlopen(
    'http://gip.itc.utwente.nl/exercises/data_sources/vehiclePositions.pb')
...
for entity in feed.entity:
    if x1wgs < entity.vehicle.position.longitude and entity.vehicle.position.longitude < x2wgs:
        if y1wgs < entity.vehicle.position.latitude and entity.vehicle.position.latitude < y2wgs:
            ...
            feature = Feature(
                geometry=Point((entity.vehicle.position.longitude,
