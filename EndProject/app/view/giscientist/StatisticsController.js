...
Ext.define('CityApp.view.giscientist.StatisticsController', {
  extend: 'Ext.app.ViewController',
  alias: 'controller.statistics-main',

  requires: ['CityApp.view.giscientist.LegendWindow'],
  ...
  initializeView: function () {
    Ext.create({
      ...
      listeners: {
        show: function () {
          ...
          this.setY(
            Ext.getCmp('sts_map_panel').getHeight() - this.getHeight() + 45
          );
        },
      },
    });
    ...
    this.stsMap = CityApp.app.homeCtrl.createMap(
      'brtachtergrondkaartgrijs',
      'sts_map_panel'
    );
    this.stsMap.getView().setCenter(CityApp.app.cityCoords);
    this.stsMap.getView().setZoom(7.5);

    var cityWMS = new ol.layer.Tile({
      source: new ol.source.TileWMS({
        url:
          'http://owsgip.itc.utwente.nl/cgi-bin/services/city_boundary?map=d:/iishome/student/s2241587/CityApp/app/api/cityboundaries.map&',
        params: {
          MAP: 'd:/iishome/student/s2241587/CityApp/app/api/cityboundaries.map',
          LAYERS: 'WMS-WFS-PostGIS',
          CITYNAME: 'Delft',
          SERVER: 'MapServer',
          TILED: true,
        },
    ...
    var districtsWMS = new ol.layer.Image({
      source: new ol.source.ImageWMS({
        ...
        params: {
          MAP:
            'd:/iishome/student/s2241587/CityApp/app/api/adminboundaries.map',
          LAYERS: 'districts',
          SERVER: 'MapServer',
          TILED: true,
        },
    ...
    Ext.getStore('districts').load({
      ...
      callback: function (records) {
        var html =
          '<span style="line-height:1.8;"><b>District Names</b></span><br/>';
        records.forEach(function (record) {
          html +=
            '<b>' + record.data.label + ':</b> ' + record.data.name + '<br/>';
        });
        Ext.getCmp('districts_legend_window').setHtml(html);

        stsCtrl.districts = records;
        stsCtrl.barChart = stsCtrl.drawBarChart(
          stsCtrl.districts,
          'sts_barchart_panel'
        );
        stsCtrl.drawPieLegend('sts_pielegend_panel');
        stsCtrl.pieChart = stsCtrl.drawPieChart(
          stsCtrl.districts,
          'sts_piechart_panel'
        );
      },
  ...
  renderVectorFeatures: function () {
    var featuresUrl =
      'https://gisedu.itc.utwente.nl/cgi-bin/mapserv.exe?map=' +
      'd:/iishome/student/s2241587/CityApp/app/api/adminboundaries.map' +
  ...
  showChoropleth: function (choroData, group, className) {
    ...
    choroLegendObj = d3
      .select('#' + choroLegendWindow.body.id)
      .append('svg')
      .attr('width', 160)
      .attr('height', 12);
    ...
    choroStyle = function () {
      return function (feature, resolution) {
        ...
        rgb = ol.color.asArray(
          colorStore.getById(group).get('tints')[tintNumber]
        );
  ...
  redrawBars: function (barsData, fillColor, strokeColor, yAxisLabel) {
    ...
    d3.select('#bc_yaxis_label').text(yAxisLabel);
  },
  /*---*/
  ...
  drawPieChart: function (districts, chartPanel) {
    ...
    function mouseOverPie(slice) {
      ...
      d3.selectAll('.legend-element').style('opacity', function (r) {
        if (r.class_code == slice.data.group) {
      ...
      CityApp.app.stsCtrl.showChoropleth(
        sliceData,
        slice.data.group,
        className
      );
    ...
    groupKeys = districts[0].data.landuse_2012
      .map(function (r) {
        return r.group;
      })
      .sort();
    (totalArea = d3.sum(districts, function (r) {
      return r.data.area_km2 * 1000000;
    })),
      (pc.cityTotals = groupKeys.map(function (key) {
        groupArea = d3.sum(districts, function (r) {
          return r.data.landuse_2012.find(function (i) {
            return i.group == key;
          }).m2;
        });
        return {
  ...
  refreshPieChart: function (pieData, districtName) {
    ...
    d3.select('#pie_title').text('Landuse in : ' + districtName);
  },
  /*---*/
  ...
  resizeBarChart: function () {
    if (this.barChart) {
      ...
      d3.select('svg#bar_chart').attr('width', bc.panel.getWidth());
  ...
  drawBarChart: function (districts, chartPanel) {
    ...
    function onMouseOverChart(record) {
      ...
      barTooltip.html(
        '<b>' +
          record.data.name +
          '</b><br />' +
          Ext.util.Format.number(record.data.pop_2016, '0,000') +
          '&nbsp;inh.' +
          '<br />' +
          Ext.util.Format.number(record.data.area_km2, '0,000') +
          '&nbsp;km&sup2;'
      );
      barTooltip
        .transition()
      ...
      var currentFeature = CityApp.app.stsCtrl.choroLayer
        .getSource()
        .getFeatures()
        .filter(function (f) {
      ...
      CityApp.app.stsCtrl.onMouseOverFeature
        .getFeatures()
        .push(currentFeature[0]);
    }
    /*---*/
    ...
    bc.y.domain([
      0,
      d3.max(districts, function (r) {
        return r.data.pop_2016;
      }) * 1.1,
    ]);
    ...
    bc.obj
      ...
      .attr('class', 'bar')
      .attr('id', function (r) {
        return 'bar_' + r.data.code;
      })
      .attr('x', function (r) {
        return bc.x(r.data.label) + (bc.x.bandwidth() * 0.2) / 2;
      })
      .attr('y', function (r) {
        return bc.y(r.data.pop_2016);
      })
      .attr('width', bc.x.bandwidth() * 0.8)
      .attr('height', function (r) {
        return bc.height - bc.y(r.data.pop_2016);
      })
      .style('stroke', '#AAA')
      .style('stroke-width', 1)
      .style('fill', '#CACCCE')
