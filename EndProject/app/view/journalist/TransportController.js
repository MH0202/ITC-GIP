...
Ext.define('CityApp.view.journalist.TransportController', {
  ...
  initializeView: function () {
    this.txpMap = CityApp.app.homeCtrl.createMap(
  ...
  getVehiclePositions: function (bboxCoords) {
    Ext.Ajax.request({
      ...
      success: function (response) {
        CityApp.app.txpCtrl.updateVehiclePositions(
  ...
  updateVehiclePositions: function (vehiclePositions) {
    ...
    vehiclePositions.features.forEach(function (vehicle) {
      ...
      if (feature) {
        /* update geomtery if the feature exists */
        feature.setGeometry(
          new ol.geom.Point([
    ...
    layerSource.forEachFeature(function (feature) {
      /* remove features that are no longer in the feed */ if (
        feature.get('timestamp') < updateTime
      ) {
  ...
  refreshVehicleLayer: function () {
    ...
    CityApp.app.txpCtrl.interval = setInterval(function () {
      CityApp.app.txpCtrl.getVehiclePositions(
        CityApp.app.txpCtrl.txpMap.getView().calculateExtent()
      );
    }, 60000);
